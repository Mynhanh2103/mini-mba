...
// Import Modal đăng ký (Đảm bảo bạn đã tạo file này ở bước trước)
...
export default function ModulesPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-800">
      {/* HEADER */}
      <header className="bg-gradient-to-r from-blue-900 to-slate-900 text-white pt-24 pb-16 px-6">
        <div className="max-w-7xl mx-auto text-center">
          ...
          <h1 className="text-4xl md:text-5xl font-extrabold mb-6">
            Kho Khóa Học & Module Chuyên Sâu
          </h1>
          <p className="text-blue-100 text-lg max-w-3xl mx-auto font-light">
            Linh hoạt lựa chọn kiến thức bạn cần. Đăng ký học theo từng Module
            riêng lẻ hoặc tích lũy để nhận chứng chỉ Mini MBA.
          </p>
        </div>
      </header>
      ...
      <div className="sticky top-0 z-30 bg-white/80 backdrop-blur-md border-b border-slate-200 shadow-sm">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between gap-4">
          <div className="font-bold text-slate-700">
            {filteredModules.length} Module có sẵn
          </div>
            ...
            {filteredModules.map((mod) => (
              ...
              >
                ...
                <div className="p-6 flex flex-col flex-grow">
                  ...
                  {/* Thông tin metadata lấy từ Admin */}
                  <div className="pt-4 border-t border-slate-100 mt-auto space-y-4">
                    <div className="flex items-center justify-between text-xs font-bold uppercase tracking-wide text-slate-400">
                      <span className="flex items-center gap-1">
                        <Clock size={14} /> {mod.duration || "4 Giờ"}
                      </span>
                      {mod.has_certificate && (
                        <span className="text-green-600 flex items-center gap-1">
                          <CheckCircle size={14} /> Có chứng chỉ
                        </span>
                      )}
                    </div>

                    {/* NÚT ĐĂNG KÝ - ĐÃ GẮN SỰ KIỆN CLICK */}
                    <button
                    ...
                    >
                      <ShoppingCart size={18} /> Đăng ký học lẻ
                    </button>
                  </div>
                </div>
              </div>
